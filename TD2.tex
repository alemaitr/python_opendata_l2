...
\formation{L2 MIASHS}
\titre{TD2 : Restaurants new-yorkais}
\begin{document}

Ce sujet traite des fichiers JavaScript Object Notation (JSON), avec export au format CSV.
%%% Déjà dit dans le TD1
% Ce format permet de stocker des données structurées, par exemple avec une organisation hiérarchique.

\section*{Préambule}
\begin{enumerate}
    \item Dans votre dossier PythonOpenData, créez un sous-répertoire TD2 dans lequel vous réaliserez l'ensemble des travaux.
...
\begin{enumerate}
    \item Sur Cursus, télécharger le jeu de données \verb+NYfood.json+ et l'enregistrer dans le dossier PythonOpenData/TD2.
...
Les questions suivantes nécessitent de manipuler les données depuis Python et le code produit devra être inclus dans un nouveau fichier \verb+td2.py+.
