...
Il est à noter que l'API qui fournit ces données peut être interrogée de la façon suivante :
...
\begin{enumerate}
    \item Écrivez une fonction qui prend en entrée un genre cinématographique et un nom (de famille) d'acteur/actrice \verb+nom_famille+ et retourne la liste des films du genre donné pour lesquels au moins un acteur ou une actrice a pour nom de famille \verb+nom_famille+.
    ...
    \item Modifiez la fonction précédente pour que si l'argument \verb+nom_famille+ n'est pas fourni lors de l'appel, la fonction retourne l'ensemble des films du genre voulu.
