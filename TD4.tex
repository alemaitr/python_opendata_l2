...
\formation{L2 MIASHS}
\titre{TD4 : Problèmes de trajets, co-voiturages et copains}
\begin{document}

\section*{Préambule}
\begin{enumerate}
    \item Dans votre dossier PythonOpenData, créer un sous-répertoire TD4 dans lequel vous réaliserez l'ensemble des travaux. 
    \item Télécharger sur Cursus l'archive contenant les données sources du TD4.
    \item Lancer l'éditeur Visual Studio Code.
    \item Dans Visual Studio Code, ouvrir le dossier PythonOpenData/TD4. 
    \item Copier le fichier credentials.json du TD3 dans le dossier PythonOpenData/TD4.
\end{enumerate}

Ce TD présente trois problèmes de calculs de trajets. Pour chacun des problèmes, on pourra utiliser des fonctions disponibles dans le fichier source TD4.py.
...
\\
    ...
    \begin{itemize}
        ...
        \item une clé d'API OpenRouteService
    \end{itemize}
...
La question peut par exemple être posée sous la forme suivante :
      ...
      \begin{itemize}
            \item Pauline, joue au tennis et au squash, se trouve "Place du recteur Henri Le Moal, Rennes, France"
            \item Ernest, joue au football et pratique la course à pied, se trouve "Place du Parlement de Bretagne, Rennes, France"
            \item Felix, joue au tennis et au football, se trouve "182 rue de l'Alma, Rennes, France"
            \item Sarah, joue au football, au squash et au tennis, se trouve "88, rue Alphonse Guérin, Rennes, France"
            \item Ingrid, pratique la course à pied, se trouve "3 Mail François Mitterrand, Rennes, France"
      \end{itemize}
